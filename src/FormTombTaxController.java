...
public class FormTombTaxController {
    ...
    public synchronized void addPerson(ActionEvent event) {
        ...
        saveToDatabaseForWorker(nameFromField, fatherNameFromField, surnameFromField, regionFromField, phoneNumFromField);
    ...
    // Save person to database
    public void saveToDatabaseForWorker(String name, String fatherName, String surname, String region, String phoneNum) {
        String insertQuery = "INSERT INTO person_worker(id, name, fatherName, surname, region, phoneNum) VALUES(?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(insertQuery)) {

            int newId = getNextPersonId(name, surname, fatherName);  // Get unique ID
    ...
    //Fix the problem such that if there exists a person with max id it starts from the max id and not the idea you had
    private int getNextPersonId(String name, String surname, String fatherName) {
        int nextId = 24000000; // Starting ID
        String sqlPersonWorkerByName = "SELECT id FROM person_tax WHERE name = ? AND surname = ? AND fatherName = ?";
        String sqlMaxPersonTax = "SELECT MAX(id) FROM person_worker";
        String sqlCheckPersonTaxById = "SELECT id FROM person_tax WHERE id = ?";
        String sqlCheckPersonWorkerById = "SELECT id FROM person_worker WHERE id = ?";
