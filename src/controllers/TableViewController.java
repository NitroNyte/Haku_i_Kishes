...
public class TableViewController {
    ...
    public ObservableList<Person_table> originalDataList;
    ...
    public void initialize() {
        ...
        //Person_table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_ALL_COLUMNS); // Ensures no extra space

       // col_id.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_name.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_fatherName.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_surname.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_region.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_outRegion.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7));
        //col_phoneNum.prefWidthProperty().bind(Bindings.divide(Person_table.widthProperty(), 7)); 

        //This is the best solution the creator could come up this, the code above doesn't work as wanted so this is the solution
   ...
   protected void updateItem(String phoneNum, boolean empty) {
    ...
    if(empty || phoneNum == null) {
        ...
        Person_table.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2 && Person_table.getSelectionModel().getSelectedItem() != null) { // Double-click
                                                                                                            // detected
    ...
    public void editPerson(ActionEvent event) {
        ...
        if (selectedPerson != null) {
            try {
                ...
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/EditPersonPanelChurchTax.fxml"));
                Parent root = loader.load();
    ...
    public void refreshTableView(ObservableList<Person_table> newPeopleList) {
        if (originalDataList != null) {
            ...
            originalDataList = FXCollections.observableArrayList(newPeopleList); // Initialize originalDataList if it's
                                                                                 // null
    ...
    public void openSearchForm(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/SearchFormTransaction.fxml"));
            Parent root = loader.load();
    ...
    public void sendToPayment() {
        ...
        if (selectedPerson != null) {
            try {
                ...
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/PaymentPanel.fxml"));
                Parent root = loader.load();

                // Get the controller and pass the selected person ID
                PaymentController paymentController = loader.getController();
                paymentController.setPersonDetails(selectedPerson.getId(), selectedPerson.getName(),
