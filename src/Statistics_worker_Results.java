...
public class Statistics_worker_Results {
    ...
    public String getYearSum(String selectedYearNum){
        String totalNum = "";
        String sqlYearSum = "SELECT SUM(CAST(payment AS INTEGER)) FROM payment_worker WHERE SUBSTR(paymentData, 7, 4) = ? ";
...
totalNum = String.valueOf(rsYearSum.getInt(1));
    ...
    public String getPeopleThatPaid(String peopleThatPaid){
        String totalNum = "";
        String sqlPeopleThatPaidSum = "SELECT COUNT(*) FROM payment_worker WHERE SUBSTR(paymentData, 7, 4) = ?";
        try (Connection conn = DatabaseConnection.getConnection();
